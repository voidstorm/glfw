...
void _glfwInputWindowSizeExit(_GLFWwindow* window, int width, int height) {
    assert(window != NULL);
    assert(width >= 0);
    assert(height >= 0);

    if (window->callbacks.sizeExit)
        window->callbacks.sizeExit((GLFWwindow*)window, width, height);
}
...
GLFWAPI GLFWwindowsizefun glfwSetWindowSizeExitCallback(GLFWwindow* handle,
    GLFWwindowsizefun cbfun) {
    _GLFWwindow* window = (_GLFWwindow*)handle;
    assert(window != NULL);

    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWwindowsizefun, window->callbacks.sizeExit, cbfun);
    return cbfun;
}
